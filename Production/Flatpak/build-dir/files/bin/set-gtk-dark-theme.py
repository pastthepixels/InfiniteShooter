#!/usr/bin/env python3
"""
Set the GTK titlebar to dark.

I modified lines 75 > end

Modified from (the only thing I did was s/spotify/InfiniteShooter/g)
https://github.com/flathub/com.spotify.Client/blob/master/set-dark-theme-variant.py

but I got the idea from
https://github.com/flathub/com.discordapp.Discord/blob/master/set-gtk-dark-theme.py
"""
...
if __name__ == '__main__':
    if 'INFINITESHOOTERFLATPAK_DISABLE_DARK_TITLEBAR' in os.environ:
        sys.exit(0)

    start = time.time()
    # Listen for X Property Change events.
    root.change_attributes(event_mask=Xlib.X.PropertyChangeMask)
    window_titles = ('InfiniteShooter', 'InfiniteShooter (DEBUG)')
    variant = 'dark'
    ...
    if not set_theme_variant(window_titles, variant):
        # Basically give our window 10 (modified!) secs to show up.
        ...
        while True:
            if time.time() - start <= 10:
                ...
                # there has been a Property Change event. --> Actually, no! I modified the script so that it tries to set the theme variant as fast as possible (I commmented line 83)
